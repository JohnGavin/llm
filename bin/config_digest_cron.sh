...
if ! wait_for_resolvable_host "" log; then
  ...
  if command -v duckdb >/dev/null 2>&1 && [ -f "${UNIFIED_DB}" ]; then
    _defer_id="$(python3 -c 'import uuid; print(str(uuid.uuid4()))')"
    _defer_ts="$(python3 -c 'import datetime; print(datetime.datetime.utcnow().isoformat() + "Z")')"
    duckdb "${UNIFIED_DB}" "
      ...
      VALUES (
        '${_defer_id}',
        'config_digest',
        '${SCRIPT_DIR}/config_digest_cron.sh',
