#!/usr/bin/env bash
# bin/roborev_install_commit_msg_hook_all.sh
#
# Wrapper: iterates ~/docs_gh/* looking for git repositories and calls
# roborev_install_commit_msg_hook.sh on each one.
#
...
#   bin/roborev_install_commit_msg_hook_all.sh
#
# Part of: JohnGavin/llm#352 — all-repos commit-msg hook installer wrapper

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALLER="${SCRIPT_DIR}/roborev_install_commit_msg_hook.sh"
