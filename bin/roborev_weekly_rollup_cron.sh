...
# shellcheck disable=SC1091
source "${REPO_DIR}/.claude/scripts/wait_for_resolvable_host.sh"
...
if [ ! -e "${GCROOT_DRV}" ] || [ ! -e "${GCROOT_STAMP}" ] || [ "${LLM_NIX}" -nt "${GCROOT_STAMP}" ]; then
  "${REPO_DIR}/.claude/scripts/nix_gcroot_refresh.sh" "${LLM_NIX}" >> "${LOG}" 2>&1 || true
fi
...
"$NIX_SHELL_BIN" "$NIX_TARGET" --run "Rscript '${ROLLUP_SCRIPT}'" 2>&1 | tee -a "$LOG"
...
"$NIX_SHELL_BIN" "$NIX_TARGET" --run "Rscript '${EMAIL_SCRIPT}'" 2>&1 | tee -a "$LOG"
