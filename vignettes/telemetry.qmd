...
```{r dashboard-status, results="asis", echo=FALSE}
if (!is.null(daily_data) && nrow(daily_data) > 0) {
  summarize_llm_usage(daily_data) |>
    gt() |>
    tab_header(title = "Usage Status", subtitle = "Current usage against limits") |>
...
#| fig-height: 10
if (!is.null(daily_data) && nrow(daily_data) > 0) {
  p1 <- get_model_breakdown(daily_data) |>
    ggplot(aes(x = reorder(modelName, total_cost), y = total_cost)) + 
...
tryCatch({
  ...
  tibble(path = files) |>
    ...
    count(ext, sort = TRUE) |>
    head(10) |>
    kable(caption = "Top 10 File Types")
}, error = function(e) cat("Error reading files"))
```
...
stats <- tryCatch({
  ...
  tibble(
    ...
    Value = c(
      as.character(info$stargazers_count),
      as.character(info$forks_count),
      as.character(info$open_issues_count),
