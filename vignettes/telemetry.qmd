...
- **LLM Usage Statistics** (Claude + Gemini)
- Session information
...
The following dashboard provides real-time visualization of LLM consumption against daily and weekly limits.

```{r llm-dashboard}
#| results: "asis"
#| comment: ""
library(llm)
withr::with_options(list(cli.num_colors = 1, cli.dynamic = FALSE), {
...
Analysis of Gemini CLI usage parsed from local session logs.
...
if (has_gemini) {
  ...
  gm_sessions <- dbGetQuery(con, "SELECT * FROM sessions_summary ORDER BY last_updated DESC")
  dbDisconnect(con, shutdown = TRUE)
}
```

```{r gemini-cost-chart}
#| fig-cap: "Daily Gemini Costs"
if (has_gemini && nrow(gm_daily) > 0) {
  ggplot(gm_daily, aes(x = as.Date(date), y = total_cost)) +
    geom_col(fill = "#4fc3f7", alpha = 0.7) +
    geom_smooth(method = "loess", se = FALSE, color = "#ff9800") +
    scale_y_continuous(labels = scales::dollar_format()) +
    labs(title = "Estimated Gemini Daily Costs", x = "Date", y = "Cost (USD)") +
    theme_minimal()
} else {
...
```{r gemini-token-chart}
#| fig-cap: "Gemini Token Trends"
if (has_gemini && nrow(gm_daily) > 0) {
  ggplot(gm_daily, aes(x = as.Date(date), y = total_tokens / 1e6)) +
    geom_line(color = "#4fc3f7", linewidth = 1) +
    geom_point() +
    labs(title = "Gemini Token Consumption", x = "Date", y = "Tokens (Millions)") +
    theme_minimal()
}
```
...
Historical usage data from Claude Code (`ccusage`).

```{r claude-setup-vignette}
daily_data <- load_cached_ccusage("daily", project_filter = NULL)
```

```{r claude-cost-trend}
#| fig-cap: "Daily Claude Costs"
if (!is.null(daily_data) && nrow(daily_data) > 0) {
  ...
  ggplot(cost_trend, aes(x = date, y = daily_cost)) +
    ...
    scale_y_continuous(labels = scales::dollar_format()) +
    labs(title = "Daily Claude Costs", x = "Date", y = "Cost (USD)") +
    theme_minimal()
}
```
...
Direct capture from the system monitor tool.

```{r claude-monitor-run}
#| comment: ""
monitor_path <- here::here("inst/extdata/cmonitor_daily.txt")
if (file.exists(monitor_path)) {
  cat(paste(readLines(monitor_path), collapse = "\n"))
} else {
  cat("No monitor data captured.")
}
```
...
```{r llm-setup-historical}
# Load llm package
library(llm)
library(here)
library(scales)

# Check if gt packages are available
...
if (has_gt) {
  ...
  message("gt/gtExtras not available - using basic tables")
}
...
This section tracks Claude Code usage via two complementary tools:
- **ccusage**: Project-level historical analysis (JSON export, DuckDB-friendly)
- **claude-monitor**: Real-time monitoring with burn rate predictions

### Real-time Usage (claude-monitor)

```{r claude-monitor-setup}
#| cache: false

# Capture claude-monitor daily view
capture_claude_monitor <- function(view = "daily", plan = "max5") {
  tryCatch({
    output <- system2(
      "claude-monitor",
      args = c("--view", view, "--plan", plan, "--no-version"),
      stdout = TRUE,
      stderr = TRUE,
      timeout = 10
    )
    # Remove ANSI escape codes
    gsub("\\x1b\\[[0-9;]*m", "", output)
  }, error = function(e) NULL)
}

monitor_output <- capture_claude_monitor("daily", "max5")
```

```{r claude-monitor-parse}
#| message: false

# Parse the summary section from claude-monitor output
parse_monitor_summary <- function(output) {
  if (is.null(output)) return(NULL)

  # Find summary section
  summary_lines <- output[grepl("Total Tokens:|Total Cost:|Entries:", output)]

  if (length(summary_lines) == 0) return(NULL)

  # Extract values using regex
  total_tokens <- stringr::str_extract(
  ...
  total_cost <- stringr::str_extract(
    paste(output, collapse = " "),
    "Total Cost: \\$([0-9,.]+)"
  ) |> stringr::str_extract("[0-9.]+") |> as.numeric()

  entries <- stringr::str_extract(
    paste(output, collapse = " "),
    "Entries: ([0-9,]+)"
  ) |> stringr::str_extract("[0-9]+") |> as.numeric()

  tibble(
    Metric = c("Total Tokens (30 days)", "Total Cost (30 days)", "Entries"),
    Value = c(
      scales::comma(total_tokens),
      scales::dollar(total_cost),
      scales::comma(entries)
    )
  )
}

monitor_summary <- parse_monitor_summary(monitor_output)

if (!is.null(monitor_summary)) {
  knitr::kable(monitor_summary, caption = "claude-monitor Summary (Last 30 Days)")
} else {
  cat("claude-monitor not available or no data.\n")
}
```

### Interactive Dashboard and Progress Bars

The following dashboard provides real-time visualization of LLM consumption against daily and weekly limits, as well as Max5 block tracking.

```{r llm-dashboard-interactive}
#| results: "asis"
#| comment: ""

if (!is.null(daily_data) && nrow(daily_data) > 0) {
  # Create a static summary for the report instead of the interactive CLI dashboard
  ...
  if (has_gt) {
    ...
    knitr::kable(summary_stats, caption = "LLM Usage Status")
  }
} else {
...
# Force cli output for Quarto
withr::with_options(list(cli.num_colors = 1, cli.dynamic = FALSE), {
  get_block_history(days = 3)
})
```

### Historical Usage (ccusage)
...
if (!is.null(daily_data) && nrow(daily_data) > 0) {
  ...
  if (has_gt) {
    ...
    knitr::kable(summary_stats, caption = "LLM Usage Summary")
  }
} else {
...
if (!is.null(daily_data) && nrow(daily_data) > 0) {
  ...
  if (!is.null(model_costs) && nrow(model_costs) > 0) {
    if (has_gt) {
      ...
      knitr::kable(model_costs, caption = "Model Usage Breakdown")
    }
  }
}
```
...
if (!is.null(session_data) && nrow(session_data) > 0) {
  ...
  if (has_gt) {
    ...
    knitr::kable(session_display, caption = "Session Usage")
  }
} else {
...
if (!is.null(daily_data) && nrow(daily_data) > 0) {
  ...
  if (!is.null(gaps) && nrow(gaps) > 0) {
    gaps_display <- gaps |> arrange(desc(gap_start))
    ...
    if (has_gt) {
      ...
      knitr::kable(gaps_display, caption = "Activity Gaps")
    }
  } else {
...
This section analyzes long-term trends in session duration and cost efficiency to investigate if sessions are becoming shorter or more expensive over time.

```{r session-efficiency}
#| fig-cap: "Trends in session duration and cost efficiency"
#| fig-height: 10
...
if (!is.null(blocks_data) && !is.null(blocks_data$blocks)) {
  ...
  daily_trends <- session_metrics |>
    ...
    summarise(
      ...
      avg_cost_per_min = mean(cost_per_min, na.rm = TRUE),
  ...
  p2 <- ggplot(daily_trends, aes(x = date, y = avg_cost_per_min)) +
    geom_point(alpha = 0.5, color = "darkgreen") +
    geom_smooth(method = "loess", se = FALSE, color = "darkolivegreen") +
    scale_y_continuous(labels = dollar_format()) +
    labs(
      title = "Cost per Minute Over Time",
      subtitle = "Is the burn rate increasing?",
      x = "Date",
      y = "Avg Cost / Minute ($)"
    ) +
    theme_minimal()
  ...
  # Plot 3: Correlation
  p3 <- ggplot(session_metrics, aes(x = duration_mins, y = cost_per_min)) +
    ...
    labs(
      ...
      subtitle = "Do shorter sessions have higher burn rates?",
      x = "Duration (mins, log scale)",
      y = "Cost/Min ($, log scale)"
    ) +
    theme_minimal()
  ...
  cat("No blocks data available for efficiency analysis.\n")
}
```
