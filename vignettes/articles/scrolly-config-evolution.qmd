...
image: ../../man/figures/vignette-scrolly-config-evolution.svg
format:
  closeread-html:
    toc: false
    code-fold: true
...
if (nrow(cfg) == 0L) {
  # facet_wrap(~category) errors with "Faceting variables must have at
  # least one value" on a zero-row frame (#889) — degrade to a blank
  # placeholder panel rather than aborting the whole site build.
  ggplot() +
    labs(
      title    = "Line-count distribution by category",
      subtitle = "No config file data available for this build"
    ) +
    base_theme
} else {
