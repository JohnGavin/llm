...
# entire purpose.
#
# Backup lifecycle (llm#1196): a transient `<cache_file>.bak-<UTC
# timestamp>` is written as a safety net while installing, then DELETED
# once the install is verified (present, mode 600, header intact) --
# NEVER left behind on a normal run. A surviving `.bak-*` file after this
# script exits means verification failed and is a genuine anomaly to
# investigate, not routine accumulation (six had built up by 2026-09-13,
# #1188, before this fix — one new file per regen, forever).
#
...
# _file_mode FILE — prints the octal mode on success and returns 0.
# Returns 1 (prints nothing) when the mode could not be determined (file
# missing, stat unavailable) — an error path and a negative result must
# never share an exit (checks-must-distinguish-unknown); callers MUST
# check the exit status, not just test the printed value against "600".
_file_mode() {
  local f="$1" out
...
_install_cache() {
  ...
  # Backs up any existing cache to a TRANSIENT timestamped file (uniqueness
  # matters here only to avoid a same-second collision with a concurrent
  # invocation, not to keep history), installs atomically, chmod 600,
  # prepends the GENERATED header, ensures a trailing newline. Once the
  # install is VERIFIED (file present, mode 600, header intact), the
  # transient backup is deleted -- llm#1196: a new timestamped backup on
  # every single run left six accumulated by 2026-09-13 (#1188), and the
  # secret-exposure-scan "no sibling copies" invariant (llm#1196) would
  # otherwise fire on that accumulation as the norm rather than the
  # anomaly it is supposed to be. If verification fails, the backup is
  # deliberately LEFT IN PLACE -- that is a genuine anomaly worth a human
  # looking at, not something to silently discard.
  #
  # Prints the backup path only when it survives (i.e. verification
  # failed); prints an empty string on a normal, verified install.
  ...
  if [ -n "$backup" ]; then
    # Capture _file_mode's exit status explicitly rather than only testing
    # its printed value -- an indeterminate mode (stat failed) must not be
    # silently treated the same as "mode is 600, verification passed".
    # Either way the backup is kept when uncertain (the safe direction),
    # but the branch itself must be deliberate, not accidental.
    local installed_mode mode_rc
    installed_mode="$(_file_mode "$cache_file")"
    mode_rc=$?
    if [ -s "$cache_file" ] \
      && [ "$mode_rc" -eq 0 ] && [ "$installed_mode" = "600" ] \
      && grep -q "GENERATED by secrets_cache_regen.sh" "$cache_file"; then
      rm -f "$backup"
      backup=""
    fi
  fi
...
EOF
  ...
  *)
  ...
  # Case: install — atomic, headers present, mode 600, and the value
  # legitimately DOES end up in the installed FILE (proving the cache
  # mechanism works even though the report never printed it). llm#1196:
  # on a VERIFIED successful install, the transient backup must be gone --
  # not merely present-then-forgotten, which is exactly how six of them
  # accumulated by 2026-09-13 (#1188).
  ...
  if [ -z "$backup_path" ]; then
    pass=$((pass + 1))
    echo "  PASS [_install_cache reports no surviving backup on a verified install]"
  else
    fail=$((fail + 1))
    echo "  FAIL [_install_cache reports no surviving backup on a verified install]: got '$backup_path'"
  fi

  local leftover_baks
  leftover_baks="$(find "$tmpdir" -maxdepth 1 -name "installed.env.bak-*" 2>/dev/null | wc -l | tr -d ' ')"
  if [ "$leftover_baks" = "0" ]; then
    pass=$((pass + 1))
    echo "  PASS [no .bak-* file left beside the cache after a verified install (llm#1196)]"
  else
    fail=$((fail + 1))
    echo "  FAIL [no .bak-* file left beside the cache after a verified install (llm#1196)]: found $leftover_baks"
  fi
  ...
  # Run install a SECOND time on top of the first -- if a backup were
  # created and left behind on every run (the pre-fix bug), two runs would
  # leave two .bak-* files. This asserts the invariant holds across
  # repeated regens, not just once (llm#1196; #1188 found six accumulated).
  local backup_path2
  backup_path2="$(_install_cache "$fewer" "$install_target")"
  local leftover_baks2
  leftover_baks2="$(find "$tmpdir" -maxdepth 1 -name "installed.env.bak-*" 2>/dev/null | wc -l | tr -d ' ')"
  if [ -z "$backup_path2" ] && [ "$leftover_baks2" = "0" ]; then
    pass=$((pass + 1))
    echo "  PASS [repeated regens do not accumulate .bak-* files (llm#1196 regression check)]"
  else
    fail=$((fail + 1))
    echo "  FAIL [repeated regens do not accumulate .bak-* files (llm#1196 regression check)]: backup_path2='$backup_path2' leftover=$leftover_baks2"
  fi
