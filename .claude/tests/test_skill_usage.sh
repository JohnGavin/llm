...
FRESHNESS_ROW=$(dq "$TESTDB" "SELECT COUNT(*) FROM etl_freshness WHERE source_name='skill_usage';")
assert "etl_freshness: skill_usage row exists" "$FRESHNESS_ROW" "1"

FRESHNESS_RUN_TS=$(dq "$TESTDB" "SELECT COUNT(*) FROM etl_freshness WHERE source_name='skill_usage' AND last_etl_run_ts IS NOT NULL;")
