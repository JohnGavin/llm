...
# UK Postcode (full EXAMPLE format like SW6 7SX, EC2Y 8NH -- fixture shapes,
# not real addresses; word "EXAMPLE" added 2026-08-22 so this doc comment is
# recognised by private_data_scan.sh's generic-pattern fixture-context
# exemption, the general-purpose mechanism any file can use -- see
# private-data-scanning.md's "Self-reference exemption scope" for why this
# file gets that fix instead of being added to private_data_scan.sh's own
# narrow, scanner-source-only exemption list (JohnGavin/llm#1004))
