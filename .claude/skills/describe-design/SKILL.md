...
methods when available (use a Haiku Explore subagent):
...
1. **Confirm the file location before writing.** You may propose a path based on repository
   ...
   without explicit user confirmation of the location.
...
```mermaid
flowchart TD
    A[_targets.R] --> B[R/tar_plans/]
    B --> C[plan_data.R]
    B --> D[plan_model.R]
    B --> E[plan_report.R]
    C --> F[(DuckDB)]
    D --> G[crew workers]
    E --> H[Quarto vignettes]
```
...
- **Describe, never copy**: Explain what code does and where to find it.
...
- **architecture-planning** - Planning new architectures (this skill documents existing ones)
- **llm-package-context** - Generating package API docs with pkgctx
