...
| 15d | Consolidated Staleness Heartbeat | Reads the `staleness_status` view (recomputed at read time from facts in the `staleness` table, populated by `.claude/scripts/staleness_collect.sh`). Supersedes the retired Phase 15c (`etl_freshness` registry status): 15c could only detect a producer that still ran but was late; a stopped producer was structurally invisible to it. 15d catches both, using `etl_freshness`'s own fact columns (`last_row_ts`, `last_etl_run_ts`, `expected_cadence_hours`) as one of its inputs. Skipped if `CLAUDE_STALENESS_CHECK=0`. 5s timeout, fail-open. See JohnGavin/llm#893. | Silent when nothing is stale; one `staleness: …` line when the collector's own heartbeat or another tracked source is stale |
| 15e | Rule Scoping Audit | Re-runs `check_rule_scoping.sh` against the canonical rules dir — the session-start backstop for `rule_scoping_precommit.sh` (that hook only fires on commits made from this checkout; a mandatory-rule-loading defect can also arrive via a pulled merge, stash pop, or direct edit). Only surfaces the safety-direction findings (`MANDATORY-BUT-*`, checker exit 3); the noisy context-bloat direction (exit 1) is left to `/check` and the pre-commit WARN path. Skipped if `CLAUDE_RULE_SCOPING_CHECK=0`. 5s timeout, fail-open. See `rule-scoping-guard` rule and JohnGavin/llm#952. | Silent when clean; cached `MANDATORY-BUT-*` finding lines when a mandatory rule is not loading |
...
- [`rule-scoping-guard`](rule-scoping-guard.md) — Phase 15e's companion trigger (pre-commit) + the full exit-code contract it shares with Phase 15e
