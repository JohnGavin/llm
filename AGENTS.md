...
**Auto-Merge Policy:** OFF (default — global toggle, all projects). PR merge stays Class C (explicit "merge" verb required every time) while this is OFF. Set to ON only on explicit user instruction, to opt merges into Class D (proceed automatically) IFF CI is fully green, the roborev merge-gate/consistency-check reports a genuine pass (never an indeterminate result treated as a pass), AND the diff touches none of the Auto-Merge Exclusion List paths (`.claude/hooks/**`, `.claude/rules/**`, credential-handling scripts, `default.nix`/`default.R`/`.claude/settings.json`, credential/secret files, DB schema/migration files, public-facing published content). Repo visibility (public/private) is NOT a criterion — private repos are often more sensitive, not less. See `human-in-the-loop-decision-points` § Conditional Auto-Merge for the full mechanism and exclusion list, and `pr-shipping-discipline` for the PR-vs-merge decision table.
...
**Mandatory rules** (auto-loaded — safety-critical, fire on every session): `verification-before-completion`, `systematic-debugging`, `btw-timeouts`, `bash-safety`, `nix-agent-shell-protocol`, `worktree-location`, `agent-identity-and-task-scopes`, `human-in-the-loop-decision-points`. Plus `auto-delegation` (governs every dispatch decision).
