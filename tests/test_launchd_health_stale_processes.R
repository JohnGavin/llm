...
test_that("extract_timeout_budget reads a plain `timeout N` prefix", {
  expect_equal(
    extract_timeout_budget("timeout 30 /opt/homebrew/bin/signal-cli -a +447521254904 receive"),
    30L
  )
})
...
test_that("extract_timeout_budget reads a `timeout -k <grace> <N>` prefix", {
  expect_equal(
    extract_timeout_budget("timeout -k 10 30 /opt/homebrew/bin/signal-cli -a +447521254904 receive"),
    30L
  )
})
...
test_that("redact_phone_numbers removes the Signal account number", {
  cmd <- "timeout 30 signal-cli -a +447521254904 --output=json receive"
  redacted <- redact_phone_numbers(cmd)
  expect_false(grepl("447521254904", redacted, fixed = TRUE))
...
test_that("a timeout-wrapped process alive far past its budget is flagged", {
  proc_table <- data.frame(
    ...
    command = "timeout 30 /opt/homebrew/bin/signal-cli -a +447521254904 --output=json receive",
...
test_that("the flagged row's command is phone-redacted", {
  ...
  stale <- detect_stale_processes(proc_table)
  expect_false(grepl("447521254904", stale$command, fixed = TRUE))
})
...
test_that("a mixed table flags only the wedged row", {
  proc_table <- data.frame(
    ...
    command = c(
      "timeout 30 /opt/homebrew/bin/signal-cli -a +447521254904 --output=json receive",
