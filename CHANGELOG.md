...
## 2026-07-22 — cross-project session: micromort deploy+contrast, best-stats-ideas repo, tennis, KB hygiene
...
- **best-stats-ideas** — new PUBLIC repo (https://github.com/JohnGavin/best-stats-ideas) from `proj/stats/ideas`; published only the plan + notes, excluded all symlinks (2 into private `llm`, 1 into clinical `Datashare/IPI`) and `.DS_Store`. Added a "further candidates" subsection + moved EDA/randomized-experiments into Foundations. Later removed `PerformanceCurves_db.md` to a local archive outside git on request.
- **tennis** (local private repo) — added the 20 Jul session (DB=6), removed the redundant Trends page (interactive grouped Overview supersedes it), hardened `update_db.R` to resolve repo root from its own path (`commandArgs`) so it runs from any cwd.
