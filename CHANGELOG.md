...
### Completed
- **roborev "did not run" backlog root-caused and cleared**: sampled the 22 open "agent could not read its own diff" reviews directly from `~/.roborev/reviews.db` — all `agent='gemini'`; traced to gemini-cli's `context.fileFiltering.respectGitIgnore` (default `true`) refusing to read roborev's own gitignored `.roborev/*.diff` snapshot. Bulk-closed all 23 currently-open reviews matching the signature with reason comments, switched `default_agent` to `claude-code`, and set `respectGitIgnore: false` in `~/.gemini/settings.json`. Posted findings to [llm#1123](https://github.com/JohnGavin/llm/issues/1123).
- **Upstream fix discovered and reconciled**: the same failure class was independently filed at [kenn-io/roborev#1104](https://github.com/kenn-io/roborev/issues/1104) (from this account, an earlier session) and fixed via [#1120](https://github.com/kenn-io/roborev/pull/1120) the same day — server-side no-verdict classification + automatic backup-agent failover, not yet in a tagged release (`v0.67.0` still latest; `roborev_weekly_update.sh` will pick it up automatically). Corrected an earlier claim in this session that roborev is closed-source — the repo is public and issues get acted on quickly. Follow-up comment posted to llm#1123.
- **CodexBar menu-bar investigation** ([llmtelemetry#365](https://github.com/JohnGavin/llmtelemetry/issues/365), filed there rather than `llm` per `private_repo_detail_guard.sh`): explicit recommendation against forking/reimplementing the 60-provider Swift app; CodexBar's `usage`/`cost --provider all --json` is per-*provider*, not per-*project*, so it cannot answer the actual ask ("usage by project and time") — that needs `ccusage` (already installed, already reads Claude Code's own per-project session logs). Phased plan: (1) surface what's already computed nightly (roborev opened/closed, lessons-learnt) with zero new data pipeline, (2) `ccusage`-driven sparklines by project/day, (3) optional CodexBar quota gauges for the 3 providers actually enabled. Also explained the "Fable only 100% left" line: an unused per-model weekly quota bucket, not a bug.
- **roborev dashboard button fixed** ([#1174](https://github.com/JohnGavin/llm/pull/1174), merged): the "View Full roborev Dashboard" email button always linked to the private companion repo, never to a local render, and its default local path (`_site/index.html`) turned out to be a CI-only artifact that had never existed on this machine. `resolve_dashboard_links()`'s default now points at the roborev-specific, self-contained vignette; `dashboard_cta_block()`/`effective_dashboard_url()` check `file.exists()` at send-time and use a `file://` href when the render is actually present, falling back to the repo URL otherwise. Both branches verified (parse + source + call, falsified with a throwaway placeholder file).
- **Launchd job survey** (user: "there are 40 chron jobs. review to see if this can be simplified") — forked a read-only survey of the ~38 jobs that are ours (of 49 total plists; the rest are third-party apps). Verdict: the job count itself is not the problem — the 14-job roborev cluster and 9 daily "pulse" jobs are each genuinely single-purpose per the documented 3-tier coverage model, no consolidation candidate found. Two real, previously-undetected bugs found and fixed in the same PR #1174:
  - `com.johngavin.roborev-failure-alert` was failing on **every single 30-minute run**: `grep -oE '[0-9]+ failed'` matched against the whole `roborev status` output also matched per-job lines in "Recent Errors" (`"job 13390 - job 13390 failed after 3 retries"`), producing a multi-line value that broke `dfail=$(( fail - pfail ))` arithmetic. Fixed by scoping the grep to the `^Jobs:` summary line only; falsified against a mock multi-line status blob before and after. Also added the plist's missing `StandardOutPath`.
  - `com.roborev.auto-refine` logged `not in a git repository` on every restart since 2026-08-18 (17+ occurrences) — non-fatal, but caused by no `WorkingDirectory` in the plist (launchd's default cwd `/` isn't a git repo). Added `WorkingDirectory` → the `llm` repo; this plist previously had no tracked source in the repo at all (unlike every sibling), so one was added.
  - Both fixes merged, pulled into the main checkout, and verified live: `launchctl list com.johngavin.roborev-failure-alert` now shows `LastExitStatus = 0`, state file freshly written.
  - The one remaining candidate finding (two digest emails sharing an 08:05 timestamp) was a false alarm on inspection — both plists' own header comments show they're deliberately staggered 5 minutes after the 08:00 roborev email to avoid nix-shell resource contention, and are genuinely different content (config-change digest vs. knowledge-base digest).

### Failed Approaches
- N/A this session — no dead ends, though one live finding (roborev "closed-source" claim) had to be corrected mid-session after new evidence (the upstream issue/PR) arrived.

### Accuracy / Metrics
- roborev: `roborev_consistency_check.sh --verbose` → `roborev:consistent (backlog=71, overview_total=2, verdicts=2, crash=0, quota=0)`. `verdicts.failed=10` vs `verdicts.addressed=4` (open backlog, not a session-blocking crash/quota signal).
- `com.roborev.auto-refine`: `not in a git repository` error eliminated on restart; a different, likely-benign roborev-internal message (`refusing to refine on main branch without --since flag`) now appears in its place — not investigated further this session.

### Known Limitations
- The roborev-specific dashboard vignette (`llmtelemetry/vignettes/roborev_summary.qmd`) has still never been rendered locally — that repo's minimal "app shell" `default.nix` has no `quarto`, `plotly`, or `DT`. The email button now falls back correctly to the repo URL until that's addressed (a separate environment decision, not made this session).
- ctx.yaml cache audit: 1 OK, 9 other-version, 24 stale (>30 days) — `ctx_sync("DESCRIPTION")` not run this session; flagged to the user at session end rather than run unprompted.
- `com.roborev.auto-refine`'s new `refusing to refine on main branch` startup message is unconfirmed as benign — plausible roborev-internal safety behavior now that cwd resolves correctly, but not traced further.
...
### Completed
- **ctx_sync silent-failure fix** ([#1112](https://github.com/JohnGavin/llm/pull/1112)) — `ctx_sync()`'s stale/missing loops now wrap `generate_ctx()` in `tryCatch()`, warn instead of aborting the whole sync, and record `status="ERROR"` per-package so one bad package no longer silently drops every package after it.
- **Signal braindump channel-scoping bug fixed** ([#1114](https://github.com/JohnGavin/llm/pull/1114), [#1113](https://github.com/JohnGavin/llm/issues/1113)) — the handler resolved `group_name` but never filtered on it, so every Signal chat (not just "Notes to llm") was captured. Documented for future sessions as [[reference_signal-braindump-channel]] memory.
- **7 Signal braindumps triaged end-to-end**, each processed → routed → linked via `braindump_act.sh`: two premortem utility-bill issues (Thames Water, East London energy — both judged not material enough for a bill-reduction investigation), two tennis issues (public/private redesign + reusable artifact template; standardized metrics + bump chart), one mycare issue (personalized dashboard from artifact template), plus the channel-scoping fix itself.
- **AgentsView quality-baseline tooling** ([#1116](https://github.com/JohnGavin/llm/pull/1116), [#1118](https://github.com/JohnGavin/llm/pull/1118)) — new rule + `agentsview_quality_baseline.sh` (25 selftest + 18 integration tests) implementing the substantive-session filter from #1115's P1 recommendations. Caught and fixed a private-repo-name leak in a subagent's own commit message before it reached `origin` (amended + force-pushed with explicit user authorization).
- **llm#1122 fixed and deployed live**: overnight self-review email moved from 06:30 to 08:45 (was reading yesterday's staleness snapshot, 1h45m before the collector's own 08:15 run); `staleness_collect.sh`'s log-freshness picker fixed to use max mtime across stdout/stderr/derived paths instead of a stale-priority pick.
- **launchd/BTM job-naming issue corrected and partially fixed** ([#1121](https://github.com/JohnGavin/llm/issues/1121), [#1128](https://github.com/JohnGavin/llm/pull/1128), merged) — a fresh whole-machine `sfltool dumpbtm` scan found the issue's original "29 of 32 fixed" audit was wrong (11 jobs actually still show generic "bash"/"sh", not 3): 3 roborev jobs already use the correct wrapper yet still show stale names (BTM-cache issue, not a wrapper-mechanism bug — proven via `branch-gc`/`worktree-gc` as working controls); 2 jobs were invisible to the original `*.plist` glob audit because they're `*.plist.template`; 1 was outside the audited directory. Fixed the 2 templates + added a previously-untracked `codexbar-refresh` plist (subagent found and preserved its real 12h schedule from an older PR rather than inventing a placeholder, per the dispatch's explicit instruction not to fabricate). "Item from unidentified developer" reopened as a separate code-signing sub-problem (previously explicitly scoped out) — not yet attempted.
- Diagnosed AgentsView's persistent ~117-120% CPU usage down to a specific, falsifiable signature: `sample`-profiled a heavy `pthread_cond_signal`/`pthread_kill` pattern inside the binary itself; ruled out (with live evidence, not assumption) staleness, an available update, the filesystem watcher (`watch_exclude_patterns` config change made zero difference), a duplicate/competing launcher, the standalone GUI app, shell rc files, this session's own hooks, and any system-wide plist. Every respawned instance has `PPID=1` (self-daemonizing) and it returns within 1-2 minutes of being killed by every stop mechanism tried (`kill`, `launchctl bootout`, plist removal, `agentsview daemon stop`) — including once during a 20-second window with zero further `agentsview` invocations from this session. Concluded it's a genuine upstream bug, not a local config problem.
- Identified a newly-enabled "Athanasia Mowinckel" Background Task Management entry as a legitimate third-party app (Entracte, `io.drmowinckels.entracte`, installed 2026-08-04) — not generated by this session or any of its automation.
- Raised [#1117](https://github.com/JohnGavin/llm/issues/1117) (Observable/interactive-dashboard workflow gap), [#1119](https://github.com/JohnGavin/llm/issues/1119) ("boast" cross-project evidence dashboard), [#1120](https://github.com/JohnGavin/llm/issues/1120) (per-project static API), [#1124](https://github.com/JohnGavin/llm/issues/1124) (auto-trigger a package-version-bump-check hook).
- Root-caused the tlang "Arrow Validation" CI failure to a `cachix/install-nix-action` + macOS-runner Nix-daemon permission issue, confirmed as a new regression (absent from the prior 2 runs) — not yet fixed; awaiting the user's choice between re-running vs. pinning `macos-14` instead of `macos-latest`.
- Explained (not a bug) the KB digest's "11 items" figure and the 9 rules-without-wiki-context table with source-level evidence; dispatched `wiki-curator` directly against the main checkout (gitignored `knowledge/` is unreachable from any worktree — a deliberate, explicitly-flagged exception to the worktree-isolation mandate) to write 7 new wiki pages documenting the 9 previously-orphaned rules.
- Merged: [#1060](https://github.com/JohnGavin/llm/pull/1060), [#1062](https://github.com/JohnGavin/llm/pull/1062), [#1064](https://github.com/JohnGavin/llm/pull/1064), [#1112](https://github.com/JohnGavin/llm/pull/1112), [#1114](https://github.com/JohnGavin/llm/pull/1114), [#1116](https://github.com/JohnGavin/llm/pull/1116), [#1118](https://github.com/JohnGavin/llm/pull/1118), [#1125](https://github.com/JohnGavin/llm/pull/1125), [#1128](https://github.com/JohnGavin/llm/pull/1128).

### Failed Approaches
- Setting `watch_exclude_patterns` in AgentsView's `config.toml` (excluding `.git`, `node_modules`, `.claude/worktrees`, `_targets`, etc.) — a well-evidenced hypothesis (heavy worktree churn) that turned out wrong. CPU stayed at ~117% after a full restart under the new config; the log then revealed the real signature (a self-generated "already running" retry storm against its own write-lock), which no config change can touch.
- Writing wrapper/memory files directly to the main checkout instead of this session's own worktree — happened once this session (caught via `git status --short` on the main checkout, self-corrected: copied into the worktree, reverted the stray main-checkout copies, committed properly).

### Accuracy / Metrics
- roborev: 0 crash, 0 quota this window (`roborev_consistency_check.sh --json`) — clean.
- AgentsView quality-baseline: 25/25 selftest + 18/18 integration tests passing (#1118).

### Known Limitations
- AgentsView is left OFF (not launchd-registered, plist removed from `~/Library/LaunchAgents/`) pending either a vendor bug report or further live syscall-level tracing (`dtrace`/`fs_usage`, not attempted — needs elevated privileges and interactive investigation). It may self-respawn again; the mechanism generating that is still not fully identified, only ruled out down to "something inside the binary itself."
- 3 roborev launchd jobs (`roborev-autoclose`, `roborev-bridge`, `roborev-agent-health`) still show stale "bash" names in Background Task Management despite already using the correct wrapper — needs investigating `sfltool resettbtm` or a full remove+re-add cycle; a normal `bootout`+`bootstrap` does not clear it.
...
### Completed
- Issue-backlog sweep continued across Tiers 0-3 plus #932's P0-P5 priority tiers — the large majority closed with real merged fixes ([#1101](https://github.com/JohnGavin/llm/pull/1101) codex-shim dir fix, [#1102](https://github.com/JohnGavin/llm/pull/1102) roborev log line, plus the tier work carried over from before this session's compaction).
- **Code Coverage CI fully fixed** ([#1103](https://github.com/JohnGavin/llm/pull/1103), [#1108](https://github.com/JohnGavin/llm/pull/1108), [#1110](https://github.com/JohnGavin/llm/pull/1110), [#1111](https://github.com/JohnGavin/llm/pull/1111)) — first-ever coverage CI run on `main`, and it had never once passed. Five distinct root causes, found in sequence as each fix revealed the next layer: (1) hard `expect_true(file.exists(...))` checks on `.claude/`-resident companion scripts, excluded from the build by `.Rbuildignore`; (2) `covr:::show_failures()` truncating its error message to `warning.length - 9` bytes (default 1000), hiding almost all real failure content; (3) `tryCatch()` never seeing the `.Rout.fail` file because covr's own `on.exit()` cleanup deletes it during stack unwinding, before `tryCatch` sees the condition — fixed by switching to `withCallingHandlers()`, which runs in place before unwinding; (4) `R/tar_plans/*.R` (deliberately uncollated, since they contain top-level `tar_target()` calls) sourced via a `pkgload::pkg_path()` trick that only resolves under `devtools::load_all()`, not a real install — fixed by symlinking into `inst/tar_plans/` and resolving via `system.file()`, mirroring the existing `inst/scripts/` pattern; (5) DuckDB `fts`/`sqlite_scanner` extensions with no install-on-demand fallback in `R/kb_search.R`.
- 19 oversized `.claude/rules/*.md` files (2 FAIL >300 lines, 17 WARN >150 lines) split into `_companions/*-details.md` files, no content deleted ([#1109](https://github.com/JohnGavin/llm/pull/1109)) — config-size check is now fully clean.
- Diagnosed and fixed the "22 stale launchd jobs" report: `staleness_collect.sh`'s `launchd_job` upsert intermittently fails (root cause not fully pinned — reproduced clean on manual re-run, so likely transient DB-lock contention around the 08:15 collector's scheduling window) and silently swallows the real duckdb error via `>/dev/null 2>&1`. Manually re-ran the collector to refresh the dashboard; the swallowed-error diagnosis itself is not yet hardened into the script.
- BWS credential cleanup: removed 3 confirmed-redundant duplicate secret entries; fixed a real bug in `secrets_cache_regen.sh` where the blunt shrink-refusal guard had no `--allow-removals` override, pre-empting the smarter named-removal guard.
- Issue #180 (a third-party name in a local wiki correction-tracking issue) reviewed against `public-private-repo-boundary`'s four triggers (phone/home/health/money) and left as-is per user decision — doesn't meet any trigger.

### Failed Approaches
- Local `nix-shell`-based test reproduction (`pkgbuild::build()` + `R CMD INSTALL` + `test_check()`) is **not equivalent** to the real CI environment — `coverage.yml` deliberately uses native Ubuntu R + CRAN packages via `r-lib/actions`, not Nix. Two rounds of "verified locally, still red in CI" resulted before this was recognized. Worse: even a supposedly-faithful local install-and-test reproduction can be silently wrong — `pkgload::pkg_path()`'s upward-DESCRIPTION search resolves to the *source tree* from certain working directories regardless of what's actually installed, masking the `R/tar_plans` bug entirely until reproduction was redone from a directory with no reachable `DESCRIPTION`.
- `tryCatch(covr::package_coverage(), error = ...)` to surface the real failure content — doesn't work; covr's internal temp-directory cleanup (`on.exit(unlink(...))`) fires during stack unwinding, which happens *before* `tryCatch` ever sees the condition. `withCallingHandlers()` runs its handler in place, before unwinding, and does work.
- Two dispatched agents hit the monthly Claude spend limit mid-task (reset 3pm Europe/Dublin) — resumed via `SendMessage` to the same agentId after the reset rather than re-dispatching fresh (would have paid twice for partially-complete work); one had already fully completed and merged before the notification even arrived.

### Known Limitations
- `staleness_collect.sh`'s launchd_job upsert failure is patched over (manual re-run) but not root-caused or hardened — the real duckdb error text is still swallowed by `>/dev/null 2>&1` at the call site; a future stale-jobs report will hit the same wall until this is fixed properly.
- Three more test files share the exact bare `LOAD sqlite`-with-no-fallback pattern just fixed elsewhere (`R/kb_search.R`) but weren't observed failing in the real CI log this round, so were deliberately left untouched rather than fixed speculatively — latent risk, not yet a live issue.
- roborev backlog: 40 failed verdicts / 22 addressed globally (0 crash, 0 quota this window) — an accumulated cross-repo state, not all attributable to this session's work; not blocking, per `roborev_consistency_check.sh`.
- Tier 3 (data-integrity, #792/#793/#839/#901/#730/#729) and #932's P6 (self-improvement/eval) tier were reached but not confirmed fully closed out by session end — pick up from wherever the last triage-agent report left off.
...
- **Signal Notes → braindumps:** signal-cli linked (+15550001111), launchd job every 5 min, messages flow to DuckDB + `knowledge/raw/braindumps/`. Tested end-to-end.
